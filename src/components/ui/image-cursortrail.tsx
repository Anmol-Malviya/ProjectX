...
export default function ImageCursorTrail({
  ...
  const handleOnMove = (e: React.MouseEvent<HTMLElement>) => {
  ...
  const handleOnTouchMove = (e: React.TouchEvent<HTMLElement>) => {
    const touch = e.touches[0]
    if (distanceFromLast(touch.clientX, touch.clientY) > window.innerWidth / distance) {
      const lead = refs.current[globalIndex % refs.current.length].current
      const tail =
        refs.current[(globalIndex - maxNumberOfImages) % refs.current.length]
          ?.current
  ...
  return (
    ...
    >
      {items.map((item, index) => {
        ...
        const imageAlt = typeof item === 'string' ? `image-${index}` : item.alt || `image-${index}`
